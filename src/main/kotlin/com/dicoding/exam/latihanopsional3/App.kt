...
fun manipulateString(str: String, int: Int): String {
    val containsNumber = str.filter { it.isDigit() }

    if (containsNumber.isNotEmpty()) {
        val numberInString = containsNumber.toInt()
        val result = numberInString * int
        return str.takeWhile { !it.isDigit() } + result
    } else {
        val trimStr = str.trimStart('0')
        val strResult = if (trimStr.isNotEmpty()) "$trimStr$int" else "$int"
        return strResult
    }
}
